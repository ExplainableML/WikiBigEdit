...
def layer_stats(
    ...
    with (torch.no_grad()):
        for batch_group in progress(loader, total=batch_count):
            for batch in batch_group:
                ...
                if not (torch.isnan(test).any() or torch.any(row_sum == 0)):
    ...
    if torch.any(row_sum2 == 0) or torch.isnan(test2).any():
        ...
        if torch.any(row_sum3 == 0) or torch.isnan(test3).any():
