...
def test_prediction_acc(model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=False, record_flops=False):
    ...
    if isinstance(prompts, str):
        prompts,targets = [prompts,], [targets,]
    try:
        prompt_target = ['Q: '+prompt + ' A: ' + target for prompt, target in zip(prompts, targets)]
    except:
        prompts = [p if isinstance(p, str) else '' for p in prompts]
        prompt_target = ['Q: ' + prompt + ' A: ' + target for prompt, target in zip(prompts, targets)]
    max_prompt_len = max([len(tok.encode(_)) for _ in prompt_target]) + 1
    if max_prompt_len > hparams.max_length:
        print(f'Prompt length exceeds max_length: {max_prompt_len}')
    ...
    prompt_tok = tok(
        ['Q: '+prompt for prompt in prompts],
        padding=True,
        truncation=True,
        max_length=min(hparams.max_length, max_prompt_len),
        return_tensors="pt",
    )
    num_prompt_toks = [int((i != tok.pad_token_id).sum())+2 for i in prompt_tok['input_ids']]
    ...
    with torch.no_grad():
        ...
        #print(f'Text answers full: {text_answers_full[0]}')
        #print(f'Text labels full: {text_labels_full[0]}')
        #print(f'Text answers: {text_answers}')
