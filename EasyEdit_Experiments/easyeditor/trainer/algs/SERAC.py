...
class SERAC(EditableModel):
    def __init__(self, model, config, model_constructor, classifier=None, classifier_tok=None,
        ...
        if replacement is None:
            ...
            if self.config.freeze_cntr:
                ...
                if config.model_class == "BertClassifier":
                    ...
                    self.replacement = getattr(transformers, config.model_class).from_pretrained(config.small_name, cache_dir='./hugging_cache', trust_remote_code=True)
        ...
        self.replacement.to(self.config.device) if not self.config.freeze_cntr else None
    ...
    def state_dict(self, destination=None, prefix="", keep_vars=False):
        ...
        if self.config.freeze_cntr and self.replacement is not None:
    ...
    def edit(self, batch, condition=None, detach_history=False):
        def detokenize(toks, tok):
            ...
            #tokens = tokens.masked_fill(toks == 128001, tok.pad_token_id)
            #tokens = tokens.masked_fill(toks == 128000, tok.bos_token_id)
