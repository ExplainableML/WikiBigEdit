...
class MEND(EditableModel):
    ...
    def edit(self, batch, condition=None, detach_history=False, return_factors=False, **kwargs):
        ...
        if isinstance(self.model, transformers.GPT2LMHeadModel):
            ...
            targ = "ji"
        mean_grads = {
            n: torch.einsum(f"bi,bj->{targ}", x, delta).to('cpu')
        ...
        for n, p in _inner_params(
            ...
            info_dict[f"grad/diff{idx}"] = (p.grad.to('cpu') - mean_grads[n]).norm(2).item()
            info_dict[f"grad/cos{idx}"] = F.cosine_similarity(
                p.grad.reshape(-1).to('cpu'), mean_grads[n].reshape(-1), dim=0
            ).item()
            idx += 1
        ...
        for n, p in edited_model.named_parameters():
            if n in pset:
                new_params.append(p + updates[n].to(p.dtype).to(p.device))
            else:
                new_params.append(p)
